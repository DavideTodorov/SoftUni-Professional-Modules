...
CREATE TABLE products_stores(
    ...
    CONSTRAINT `fk_productstores_product`
    FOREIGN KEY (`product_id`)
    REFERENCES `products`(`id`),
    
     CONSTRAINT `fk_productstores_store`
    FOREIGN KEY (`store_id`)
    REFERENCES `stores`(`id`)
);
...
SELECT `p`.`id`, 1
FROM `products` AS p
WHERE `p`.`id` NOT IN (SELECT product_id FROM products_stores GROUP BY product_id);
...
SELECT 
    s.`name`,
    COUNT(pr_s.`product_id`) AS product_count,
    ROUND(AVG(pr.`price`), 2) AS `avg`
FROM
    `stores` AS s
        LEFT JOIN
    `products_stores` AS pr_s ON pr_s.`store_id` = s.`id`
        LEFT JOIN
    `products` AS pr ON pr_s.`product_id` = pr.`id`
GROUP BY s.`name`
...
(SELECT COUNT(*) FROM employees AS e WHERE e.`store_id` = s.`id`) AS employees_count
FROM `stores` AS s
JOIN `addresses` AS a
ON s.`address_id` = a.`id`
JOIN `towns` AS t
ON a.`town_id` = t.`id`
WHERE (SELECT COUNT(*) FROM employees AS e WHERE e.`store_id` = s.`id`) >= 1
...
BEGIN
 ...
 SELECT CONCAT(e.`first_name`,' ', e.`middle_name`, '.',' ', e.`last_name`,' ', 'works in store for',' ', 
    ...
    JOIN `employees` AS e
    ON e.`store_id` = s.`id`
    WHERE s.`name` = store_name AND e.`salary` = 
    (SELECT MAX(`salary`) FROM `employees` AS e2 GROUP BY e2.`store_id`
    HAVING e2.`store_id` = s.`id`)
    INTO result;
...
BEGIN
    ...
    JOIN `products_stores` AS pr_s
    ON pr_s.`product_id` = pr.`id`
    JOIN `stores` AS s
    ON pr_s.`store_id` = s.`id`
    JOIN `addresses` AS a
    ON s.`address_id` = a.`id`
    SET pr.`price` = pr.`price` + (IF (a.`name` LIKE '0%', 100, 200))
