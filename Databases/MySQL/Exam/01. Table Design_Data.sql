...
CREATE TABLE `cars_drivers`(
    ...
    CONSTRAINT cpk_cars_drivers PRIMARY KEY (car_id, driver_id),
...
WHERE cl.`id` NOT IN (SELECT c.client_id FROM courses AS c GROUP BY c.client_id) AND 
...
ORDER BY count_of_courses DESC, c.`id`;
 ...
 -- 08. Regular clients
 SELECT cl.`full_name`, COUNT(c.`car_id`) AS count_of_cars, SUM(c.`bill`)
 FROM `clients` AS cl
 JOIN courses AS c
...
FROM
    ...
    clients AS cl ON c.`client_id` = cl.`id`
        JOIN
    cars ON c.`car_id` = cars.`id`
        JOIN
    categories AS cat ON cars.`category_id` = cat.`id`
ORDER BY c.`id`;
...
FROM
    courses
        JOIN
    addresses AS a ON courses.`from_address_id` = a.`id`
        JOIN
    clients AS cl ON courses.`client_id` = cl.id
        JOIN
