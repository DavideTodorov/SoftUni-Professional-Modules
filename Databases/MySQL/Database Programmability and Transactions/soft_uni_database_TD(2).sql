...
CREATE PROCEDURE usp_get_employees_salary_above(salary_to_check DECIMAL(10, 4))
BEGIN
...
BEGIN
	...
	SELECT `name`
    FROM `towns`
    WHERE LOWER(`name`) LIKE CONCAT(LOWER(start_letter), '%')
    ORDER BY `name`;
...
CREATE PROCEDURE usp_get_employees_from_town(town_to_search VARCHAR(25))
BEGIN
...
CREATE PROCEDURE usp_get_employees_by_salary_level(salary_type VARCHAR(15))
BEGIN
